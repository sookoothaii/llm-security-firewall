...
def main():
    ...
    ap.add_argument(
        "--predictions",
        required=True,
        help="Predictions JSONL (decision, delay, truth)",
    )
    ap.add_argument(
        "--ground_truth",
        help="Ground truth JSONL (optional if included in predictions)",
    )
    ap.add_argument(
        "--o", type=int, default=50, help="ERDE latency parameter (default: 50)"
    )
    ap.add_argument("--cfp", type=float, help="False positive cost (auto if not set)")
    ap.add_argument(
        "--cfn", type=float, default=1.0, help="False negative cost (default: 1.0)"
    )
    ap.add_argument(
        "--ctp", type=float, default=1.0, help="True positive late cost (default: 1.0)"
    )
    args = ap.parse_args()
    ...
    if len(predictions) != len(ground_truth):
        print(
            f"[ERROR] Prediction count ({len(predictions)}) != ground truth count ({len(ground_truth)})"
        )
        return
    ...
    # Calculate ERDE
    result = erde_score(
        predictions, ground_truth, o=args.o, cfp=args.cfp, cfn=args.cfn, ctp=args.ctp
    )

    # Standard metrics (delay-agnostic)
    preds_only = [p[0] for p in predictions]
    precision, recall, f1, _ = precision_recall_fscore_support(
        ground_truth, preds_only, average="binary", zero_division=0
    )
    ...
    print(
        f"  Parameters: cfp={result['cfp']:.4f}, cfn={result['cfn']:.2f}, ctp={result['ctp']:.2f}"
    )
