...
def sample_params(rng: random.Random, config: GeneratorConfig) -> CampaignParams:
    ...
    avg_inter_event = rng.uniform(config.inter_event_min, config.inter_event_max)

    # Sample tool diversity
    tool_diversity = rng.randint(config.tool_diversity_min, config.tool_diversity_max)
...
def generate_campaign_from_params(
    ...
    if label == CampaignLabel.BENIGN and params.max_phase >= 4:
        ...
        and params.max_phase >= 4
        and params.avg_inter_event_minutes > 120
    ):
        difficulty = Difficulty.HARD_FN
    elif (
        label == CampaignLabel.MALICIOUS
        and params.scope_mix > 0.3
        and params.pretext_type == PretextType.TESTLAB
    ):
    ...
    scope = (
        "internal"
        if params.scope_mix < 0.2
        else "mixed"
        if params.scope_mix < 0.8
        else "external"
    )
