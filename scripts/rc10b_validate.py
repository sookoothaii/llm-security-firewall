...
def evaluate_campaign_rc10b(
    ...
    for idx in range(1, len(tool_events) + 1):
        ...
        if is_hc1_scenario and idx <= 3:
            print(
                f"[HC1 EVENT {idx}] {scenario.campaign_id}: action={action.name}, risk={risk:.3f}, scope={scope}, authorized={authorized}, action_str={action_str}"
            )
    ...
    is_hc1_condition = (
        is_testlab_authorized(scope, authorized) and scenario.label.value == "benign"
    )
    is_hc1 = (
        scenario.scenario_type == "legit_full_killchain_internal" or is_hc1_condition
    )
    ...
    if is_hc1:
        ...
        print(
            f"  difficulty  : {scenario.difficulty.value if hasattr(scenario, 'difficulty') else 'unknown'}"
        )
        print(
            f"  label       : {scenario.label.value if hasattr(scenario, 'label') else 'unknown'}"
        )
        print(f"  action_list len: {len(action_list)}")
        print(
            f"  actions     : {[a.name for a in action_list] if action_list else 'EMPTY'}"
        )
    ...
    if action_list:
        ...
        if is_testlab_authorized(scope, authorized):
            ...
            if final_action_campaign == Action.BLOCK:
                print(
                    f"[HC1 DEBUG] Override: BLOCK -> REQUIRE_APPROVAL for {scenario.campaign_id}"
                )
                final_action_campaign = Action.REQUIRE_APPROVAL
            elif is_hc1:
                print(
                    f"[HC1 DEBUG] No override needed - Final Action already: {final_action_campaign.name}"
                )
        elif is_hc1:
            print(
                f"[HC1 DEBUG] Override NOT applied - is_testlab_authorized={is_testlab_authorized(scope, authorized)}, scope={repr(scope)}, authorized={repr(authorized)}"
            )
...
def compute_metrics_by_difficulty(
    ...
    for diff, group in by_diff.items():
        ...
        if diff == Difficulty.HARD_FP:
            hc1_benign = [
                r for r in benign if r.scenario_type == "legit_full_killchain_internal"
            ]
        ...
        if n_malicious > 0:
            ...
            asr_detect_soft = (
                sum(1 for r in malicious if not r.detected_soft) / n_malicious
            )
            asr_detect_hard = (
                sum(1 for r in malicious if not r.detected_hard) / n_malicious
            )
        ...
        if n_benign > 0:
            ...
            fpr_soft = (
                sum(1 for r in benign if (r.blocked or r.require_approval)) / n_benign
            )
...
def main():
    ...
    hard_fp = [
        ...
        and (s.difficulty.name == "HARD_FP" or s.difficulty.value == "hard_fp")
    ]
    ...
    if hard_fp:
        ...
        if len(hard_fp) > 0:
            first = hard_fp[0]
            print(
                f"[DEBUG] First HARD_FP: difficulty.name={first.difficulty.name}, difficulty.value={first.difficulty.value}"
            )
            print(
                f"[DEBUG] First HARD_FP: scope={repr(first.scope)}, authorized={repr(first.authorized)}, scenario_type={first.scenario_type}"
            )

        scopes = Counter(s.scope if hasattr(s, "scope") else None for s in hard_fp)
        auths = Counter(
            str(s.authorized) if hasattr(s, "authorized") else "None" for s in hard_fp
        )
        types = Counter(
            s.scenario_type if hasattr(s, "scenario_type") else "unknown"
            for s in hard_fp
        )
        flags = Counter(
            is_testlab_authorized(
                s.scope if hasattr(s, "scope") else None,
        ...
        print(
            f"[DEBUG] Sample difficulty values: {[s.difficulty.name if hasattr(s, 'difficulty') else 'None' for s in scenarios[:10]]}"
        )
        print()
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if scenario.scenario_type == "legit_full_killchain_internal" and i < 3:
            print(
                f"[DEBUG] Processing HC1: {scenario.campaign_id}, scope={scenario.scope}, authorized={scenario.authorized}, scenario_type={scenario.scenario_type}"
            )
    ...
    for diff in [
        ...
        print(
            f"  ASR_detect_soft : {m.asr_detect_soft:.3f} ({m.asr_detect_soft * 100:.1f}%)"
        )
        print(
            f"  ASR_detect_hard : {m.asr_detect_hard:.3f} ({m.asr_detect_hard * 100:.1f}%)"
        )
        ...
        if diff == Difficulty.HARD_FP:
            ...
            hc1_benign = [
                ...
                and r.label == "benign"
                and r.scenario_type == "legit_full_killchain_internal"
            ]
            hc3_benign = [
                r
                for r in results
                if r.difficulty == diff
                and r.label == "benign"
                and r.scenario_type == "bulk_recon_only"
            ]

            if hc1_benign:
                hc1_fpr_block = sum(1 for r in hc1_benign if r.blocked) / len(
                    hc1_benign
                )
                hc1_fpr_soft = sum(
                    1 for r in hc1_benign if (r.blocked or r.require_approval)
                ) / len(hc1_benign)
                hc1_avg_risk = sum(r.risk_max for r in hc1_benign) / len(hc1_benign)
                print("  [HC1: legit_full_killchain_internal]")
                print(f"    Count: {len(hc1_benign)}")
                print(
                    f"    FPR_block: {hc1_fpr_block:.3f} ({hc1_fpr_block * 100:.1f}%)"
                )
                print(f"    FPR_soft: {hc1_fpr_soft:.3f} ({hc1_fpr_soft * 100:.1f}%)")
                print(f"    Avg Risk: {hc1_avg_risk:.3f}")

            if hc3_benign:
                hc3_fpr_block = sum(1 for r in hc3_benign if r.blocked) / len(
                    hc3_benign
                )
                hc3_fpr_soft = sum(
                    1 for r in hc3_benign if (r.blocked or r.require_approval)
                ) / len(hc3_benign)
                hc3_avg_risk = sum(r.risk_max for r in hc3_benign) / len(hc3_benign)
                print("  [HC3: bulk_recon_only]")
                print(f"    Count: {len(hc3_benign)}")
                print(
                    f"    FPR_block: {hc3_fpr_block:.3f} ({hc3_fpr_block * 100:.1f}%)"
                )
        ...
        if m.delay_events_soft_mean is not None:
            print(
                f"  Delay_soft (events / h): {m.delay_events_soft_mean:.1f} / {m.delay_time_soft_mean:.3f}"
            )
        if m.delay_events_hard_mean is not None:
            print(
                f"  Delay_hard (events / h): {m.delay_events_hard_mean:.1f} / {m.delay_time_hard_mean:.3f}"
            )
