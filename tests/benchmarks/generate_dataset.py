...
def synth(
    n_benign=100, n_jlb=100, hard_neg=40, split_ratio=0.7, seed=1337, dedup_thr=0.8
):
    ...
    out = []
    for rid, t, label, _, sd, src, tags in rows:
        split = "test" if rid in test_ids else "dev"
        out.append((rid, t, label, split, sd, src, tags))
    return out
