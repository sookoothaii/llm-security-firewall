...
def main():
    ...
    if existing_session:
        ...
        print(f"[WARN] Session NOT found in database")
        ...
        if success:
            print(f"[OK] Test session created and saved!")
            print(f"   Max Phase: {memory.max_phase_ever}")
            print(f"   Risk Multiplier: {memory.latent_risk_multiplier:.3f}")
            print(f"\n[INFO] Next steps:")
            print(f"   1. Restart the proxy server (Ctrl+C and restart)")
            print(f"   2. Run this script again to verify persistence")
        else:
            print(f"[ERROR] Failed to save session!")
