...
# Load detectors
unicode_mod = load_module(
    "unicode_hardening", os.path.join(base_path, "detectors", "unicode_hardening.py")
)
encoding_mod = load_module(
    "encoding_chain", os.path.join(base_path, "normalizers", "encoding_chain.py")
)
entropy_mod = load_module("entropy", os.path.join(base_path, "detectors", "entropy.py"))
dense_mod = load_module(
    "dense_alphabet", os.path.join(base_path, "detectors", "dense_alphabet.py")
)
transport_mod = load_module(
    "transport_indicators",
    os.path.join(base_path, "detectors", "transport_indicators.py"),
)
identifiers_mod = load_module(
    "identifiers", os.path.join(base_path, "detectors", "identifiers.py")
)
homoglyph_mod = load_module(
    "homoglyph_spoof", os.path.join(base_path, "detectors", "homoglyph_spoof.py")
)
exotic_mod = load_module(
    "unicode_exotic", os.path.join(base_path, "detectors", "unicode_exotic.py")
)
policy_mod = load_module(
    "risk_weights_v2_otb", os.path.join(base_path, "policy", "risk_weights_v2_otb.py")
)
context_mod = load_module(
    "context", os.path.join(base_path, "preprocess", "context.py")
)
