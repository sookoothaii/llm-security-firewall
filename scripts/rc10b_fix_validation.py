...
class HardenedCampaignDetector(AgenticCampaignDetector):
    ...
    def detect_campaign(
        ...
        if final_score != base_score:
            print(
                f"[HARDENED] Base={base_score:.4f} | Phase={phase_depth} | Floor={floor_score:.2f} | CriticalFloor={critical_phase_floor:.2f} -> Final={final_score:.4f}"
            )
        ...
        hardened_report["action"] = (
            action.to_string() if isinstance(action, Action) else str(action)
        )
        hardened_report["is_blocked"] = (
            (action == Action.BLOCK)
...
def run_validation():
    ...
    if risk_max >= 0.8:
        ...
        print(
            f"  -> [PARTIAL] Floor greift, aber nicht aggressiv genug (Score >= {t_hard} aber < 0.8)"
        )
    else:
    ...
    if final_step in results:
        ...
        if final_risk >= 0.8:
            print(
                "  -> [SUCCESS] Dilution verhindert. Aggressive Floor greift (Score >= 0.8)"
            )
        elif final_risk >= t_hard:
            print(
                f"  -> [SUCCESS] Dilution verhindert. Block ausgelöst (Score >= {t_hard})"
            )
        else:
    ...
    if final_step in results:
        ...
        if final_risk >= 0.8:
            print(
                "Probe C (GTG-1002): [SUCCESS] Dilution-Problem behoben (High-Watermark + Aggressive Floor)"
            )
        elif final_risk >= t_hard:
            print(
                "Probe C (GTG-1002): [SUCCESS] Dilution-Problem behoben (Block ausgelöst)"
            )
        else:
