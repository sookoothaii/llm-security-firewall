    ...
    try:
        ...
        for writing in root.findall(".//WRITING"):
            ...
            entry = {
                ...
                "labels": {"self_harm": label, "abuse": 0, "unsafe_env": 0},
                "source": "erisk_xml",
                "date": date,
...
def convert_json_user(
    json_data: Dict[str, Any], label: int, target_subject: str = None
) -> List[Dict[str, Any]]:
...
def main():
    ...
    ap.add_argument(
        "--target_subjects",
        help="File with target subject IDs (JSON only, one per line)",
    )
    args = ap.parse_args()
    ...
    if format_type == "xml":
        ...
        pos_dir = (
            Path(args.positive_dir) if args.positive_dir else erisk_path / "positive"
        )
        neg_dir = (
            Path(args.negative_dir) if args.negative_dir else erisk_path / "negative"
        )
        ...
        for json_file in erisk_path.glob("*.json"):
            ...
            label = (
                1
                if "positive" in json_file.stem.lower()
                or "depression" in json_file.stem.lower()
                else 0
            )
            ...
            for submission in data:
                ...
                entries = convert_json_user(
                    submission, label=label, target_subject=target
                )
                all_entries.extend(entries)
    ...
    print(
        f"  python tools/layer15/split_data.py --input {output_path} --ratios 0.8 0.1 0.1"
    )
