...
class TestExternalReviewValidation:
    ...
    def test_false_positive_tracking(self):
        ...
        for query in legitimate_queries:
            ...
            if not decision.allowed:
                false_positives += 1
                print(f"WARNING: False positive detected: {query[:50]}...")
        ...
        print(
            f"PASS: False Positive Tracking: {false_positives}/{total_queries} FP ({false_positive_rate:.1%})"
        )
    ...
    def test_p99_latency_adversarial_inputs(self):
        ...
        # Load adversarial payloads (relative to firewall package root)
        suite_path = (
        ...
        if not adversarial_payloads:
            pytest.skip("No adversarial payloads found in suite")
        ...
        print("PASS: P99 Latency: PASS (< 200ms requirement met)")
    ...
    def test_cache_mode_switching(self):
        ...
        try:
            ...
            print("PASS: Cache Mode Switching: PASS (all modes work without restart)")
    ...
    def test_adversarial_bypass_suite(self):
        ...
        # Load adversarial test suite (relative to firewall package root)
        suite_path = (
            Path(__file__).parent.parent.parent
            / "data"
            / "gpt5_adversarial_suite.jsonl"
        ...
        if not test_cases:
            pytest.skip("No test cases found in adversarial suite")

        print(f"\nTesting {len(test_cases)} adversarial vectors...")
        ...
        for test_case in test_cases:
            ...
            expected = test_case.get("expected_detection", "should_block")
            ...
            if expected == "should_block":
                ...
                if decision.allowed:
                    ...
                    print(f"WARNING: False positive in test {test_id}")
        ...
        print("PASS: Adversarial Bypasses: PASS (0 bypasses)")
    ...
    def test_memory_usage_under_300mb(self):
        ...
        print("PASS: Memory Usage: PASS (< 300MB cap met)")
    ...
    def test_binary_size_under_15mb(self):
        ...
        binary_paths = [
            Path(__file__).parent.parent.parent / "dist" / "llm-firewall",
            Path(__file__).parent.parent.parent / "dist" / "llm-firewall.exe",
            Path(__file__).parent.parent.parent / "build" / "llm-firewall",
        ]
        ...
        if binary_found:
            ...
            print("PASS: Binary Size: PASS (< 15MB requirement met)")
        else:
    ...
    def test_redis_cloud_integration(self):
        ...
        # Check if Redis is available via MCP tool or environment
        redis_available = (
        ...
        print(
            f"Using Redis: {os.getenv('REDIS_CLOUD_HOST') or os.getenv('REDIS_HOST', 'localhost')}"
        )
        ...
        print(f"PASS: Redis Cloud: PASS (latency: {latency_ms:.2f} ms)")
        if health["redis"]:
            print(
...
if __name__ == "__main__":
    ...
    for test_name, test_func in tests:
        ...
        try:
            ...
            results.append((test_name, f"SKIP: {str(e)}"))
