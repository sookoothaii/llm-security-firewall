...
from llm_firewall.session.operator_budget import OperatorBudget
...
def malicious_campaign_events():
    ...
    events.append(ToolEvent(now + 120, "exploit_framework", "exploit", "corpA.com", True))
...
def test_malicious_full_killchain(detector, malicious_campaign_events):
    ...
    assert report["killchain"]["phase_depth"] >= KillChainPhase.EXPLOIT_DEVELOPMENT.value
...
def test_multi_target_campaign(detector, multi_target_campaign_events):
    ...
    assert "multi_target_campaign" in report["signals"] or "campaign_multi_exploit" in report["signals"]
...
def test_operator_budget_exceeded(detector):
    ...
    assert "operator_budget_exceeded" in report["signals"] or report["operator"]["budget_exceeded"], "Budget should be exceeded"
...
def test_pretext_signal_boost(detector, benign_events):
    ...
    assert report_with_pretext["combined_risk_score"] > report_no_pretext["combined_risk_score"], "Pretext should boost risk"
