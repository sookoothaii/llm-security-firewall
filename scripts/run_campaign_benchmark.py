...
def main():
    ...
    if args.generate_dataset:
        ...
        print(f"  Benign: {sum(1 for s in scenarios if s.label == CampaignLabel.BENIGN)}")
        print(f"  Malicious: {sum(1 for s in scenarios if s.label == CampaignLabel.MALICIOUS)}")
        return
    ...
    print(f"  Phase >= 3 (Exploit): {results.acsr_phase_3:.3f} ({results.acsr_phase_3*100:.1f}%)")
    print(f"  Phase >= 4 (Lateral): {results.acsr_phase_4:.3f} ({results.acsr_phase_4*100:.1f}%)")
    print(f"  Phase >= 5 (Exfil): {results.acsr_phase_5:.3f} ({results.acsr_phase_5*100:.1f}%)")
    ...
    if args.threshold_sweep:
        ...
        for r in sweep_results:
            print(f"  {r['threshold']:.2f}   | {r['asr']:.3f} | {r['fpr']:.3f} | {r['malicious_blocked']:3d}/{r['benign_blocked']:3d}")
