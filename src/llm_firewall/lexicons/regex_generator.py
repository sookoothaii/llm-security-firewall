...
def build_cluster_regexes(intents_json: Dict[str, Any],
                          max_gap: int = DEFAULT_MAX_GAP) -> List[Dict[str, Any]]:
    """
    ...
    for c in intents_json.get("clusters", []):
        ...
        for s in c.get("synonyms", []):
            ...
            out.append({
                ...
                "category": cid,
                "weight": round(base_w * 0.7, 3),  # regex channel slightly discounted vs exact AC
                "regex": rx,
                "flags": "i"
            })
    return out
