...
class CampaignGraph:
    ...
    target_phase_times: Dict[Tuple[str, KillChainPhase], float] = field(default_factory=dict)
    
    # Metrics
    max_phase_reached: KillChainPhase = KillChainPhase.INITIALIZATION
    targets_at_phase: Dict[KillChainPhase, int] = field(default_factory=lambda: defaultdict(int))
...
def get_campaign_features(graph: CampaignGraph) -> Dict[str, any]:
    ...
    return {
        ...
        "targets_at_recon": graph.targets_at_phase.get(KillChainPhase.RECONNAISSANCE, 0),
