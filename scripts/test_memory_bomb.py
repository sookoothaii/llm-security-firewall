...
def test_memory_bomb_protection():
    ...
    print(f"[1] Memory Initialized with MAX_FRAGMENT_HISTORY={memory.MAX_FRAGMENT_HISTORY}")
    ...
    # 4. Assertions
    print(f"[3] Verifying Eviction Logic...")
    ...
    assert len(data["fragment_queue"]) == 5, f"Persisted queue size is {len(data['fragment_queue'])}, expected 5"
    ...
    assert "hash_0" not in memory_loaded.fragment_set, "hash_0 present in loaded memory (should be evicted)"
