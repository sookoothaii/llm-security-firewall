...
def validate_regex_items(items: List[dict], strict=False):
    ...
    for it in items:
        ...
        if miss:
            out.append((it.get("id","<no-id>"), "err", f"missing keys: {sorted(miss)}"))
            continue
        patt = it["regex"]
...
def merge_cluster(dst: dict, add: dict, casefold=True):
    ...
    syn = dedup_strings(list(dst.get("synonyms", [])) + list(add.get("synonyms", [])), casefold=casefold)
    dst["synonyms"] = syn
    seed = dedup_strings(list(dst.get("seed_sentences", [])) + list(add.get("seed_sentences", [])), casefold=False)
    dst["seed_sentences"] = seed
    for k,v in add.items():
        if k in ("id","priority","synonyms","seed_sentences"): continue
...
def merge_intents(base_obj: dict, patch_obj: dict, casefold=True):
    ...
    if "clusters" not in base or not isinstance(base["clusters"], list): base["clusters"] = []
    if "clusters" not in patch or not isinstance(patch["clusters"], list): return base, {}
...
def main():
    ...
    if args.cmd == "merge-regex":
        base = load_json(base_p); patch = load_json(patch_p)
