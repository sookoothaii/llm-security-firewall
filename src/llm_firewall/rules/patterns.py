...
INTENT_PATTERNS: Dict[str, Pattern] = {
    ...
    "system_msg_override": re.compile(
        r"\bignore\s+the\s+previous\s+system\s+message\b",
        re.DOTALL | re.IGNORECASE
    ),
    ...
    "paywall_evasion": re.compile(
        r"\b(bypass|circumvent)\s+(paywall|subscription)\b",
        re.DOTALL | re.IGNORECASE
    ),
...
EVASION_PATTERNS: Dict[str, Pattern] = {
    ...
    "encoding_mentions": re.compile(
        r"\b(base64|rot13|hex|url-?encoded|unicode\s+escape)\b",
        re.IGNORECASE
    ),
