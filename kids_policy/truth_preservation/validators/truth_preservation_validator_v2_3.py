...
class TruthPreservationValidatorV2_3:
    ...
    def validate(
        ...
        veto_master_guard_passed = gmf_triggered <= veto_master_config['max_triggered_slots']
        ...
        if slot_anchors:
            ...
            for i, detail in enumerate(entail_details):
                ...
                if not detail['is_entailment'] and not detail['is_contradiction']:
                    ...
                    if anchor_hit:
                        ...
                        content_emb = self.sbert.encode(factual_content, convert_to_tensor=True)
                        ...
                        if anchor_count >= 2 or micro_sps >= 0.80:
                            ...
                            detail['upgrade_reason'] = f"anchors={anchor_count} + micro_SPS={micro_sps:.3f}"
            ...
            neutrals = sum(1 for d in entail_details if not d['is_entailment'] and not d['is_contradiction'])
    ...
    def _compute_slot_recall_with_anchors(
        ...
        content: str,
        slots: List[str],
        anchors: Optional[Dict[str, List[str]]]
    ) -> float:
        ...
        if anchors:
            ...
            for slot in slots:
                ...
                if slot_anchors and any(anchor.lower() in content_lower for anchor in slot_anchors):
                    present += 1
                else:
                    # Fallback: check in slot text itself
                    slot_lower = slot.lower()
                    if any(word in content_lower for word in slot_lower.split() if len(word) > 3):
                        present += 1
        else:
