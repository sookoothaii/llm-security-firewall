...
def extract_features(prompt: str, *, cheap_scores: Dict[str, float] | None = None) -> ExtractorOutput:
    """
    ...
    try:
        ...
        if obf_result and hasattr(obf_result, 'zwc_count'):
            ...
            gzip_b64_flag = int(obf.get("gzip_b64_flag", 1 if _has_gzip_magic_b64(text) else 0))
            confusable_ratio = float(obf.get("confusable_ratio", _confusable_ratio(text)))
    ...
    try:
        ...
        if sec_result and hasattr(sec_result, 'severity'):
            ...
            api_like = sum(1 for h in hits if "api_key" in str(h.get('pattern', '')).lower())
    ...
    xml_ratio = _clip01(len(re.findall(r"</?[A-Za-z][^>]*>", text)) / max(1, n_tokens))  # rough
    ...
    roleplay = len(re.findall(r"\b(role[-\s]?play|pretend to|you are now)\b", text, flags=re.I))
    ...
    mixed_lang_flag = 1 if sum(int(r > 0.05) for r in (cyr_r, arab_r, thai_r, han_r)) >= 1 else 0
    ...
    meta = {
        "n_chars": n_chars, "n_tokens": n_tokens,
        "zw_count": zw_count, "bidi_count": bidi_count, "mixed_scripts": mixed_scripts,
        "base64_score": base64_score, "hex_score": hex_score, "urlenc_runs": urlenc_runs,
        "rot13_flag": bool(rot13_flag), "gzip_b64_flag": bool(gzip_b64_flag),
        "obf_severity": obf_sev, "secrets_severity": sec_sev,
        "api_like": api_like, "jwt_like": jwt_like, "privkey_flag": bool(privkey_flag)
    }
...
_RE_API_KEY_LIKE = re.compile(r"\b(sk|api[-_]?key|token)[=:]\s*[A-Za-z0-9_\-]{16,}", re.I)
_RE_JWT_LIKE = re.compile(r"\beyJ[a-zA-Z0-9_-]{8,}\.[a-zA-Z0-9_-]{8,}\.[a-zA-Z0-9_-]{8,}\b")
_RE_PRIVKEY = re.compile(r"-----BEGIN (?:RSA|EC|OPENSSH|PRIVATE) KEY-----")
_RE_CRED_WORDS = re.compile(r"\b(password|passwort|token|secret|api[-_]?key|key)\b", re.I)
_RE_PII_WORDS = re.compile(r"\b(ssn|passport|credit\s*card|iban|social\s*security|ausweis|身份证)\b", re.I)

_RE_JAILBREAK = re.compile(r"\b(jailbreak|break\s+policy|bypass|ignore\s+instructions)\b", re.I)
_RE_IGNORE_SAFETY = re.compile(r"\b(ignore|disregard)\s+(the\s+)?(rules|safety|content\s*policy)\b", re.I)
...
_RE_SYS_PROMPT = re.compile(r"\b(system\s+prompt|developer\s+message|hidden\s+instruction)\b", re.I)
_RE_COT_REQUEST = re.compile(r"\b(chain[-\s]*of[-\s]*thought|show\s+your\s+reasoning)\b", re.I)
