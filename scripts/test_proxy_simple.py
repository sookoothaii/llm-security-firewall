...
def test_proxy_logic():
    ...
    print("  [OK] TopicFence")
    print("  [OK] RC10b Detector")
    print("  [OK] SafetyFallbackJudge")
    print("")
    ...
    print(
        f"Layer 1 (TopicFence): {'[OK] ON_TOPIC' if is_on_topic else '[BLOCK] OFF_TOPIC'}"
    )
    ...
    if not is_on_topic:
        ...
        print(
            f"Layer 2A (RC10b): {'[OK] ALLOWED' if not campaign_result.is_blocked else '[BLOCK] BLOCKED'}"
        )
        print(
            f"  -> Score: {campaign_result.score:.3f}, Decision: {campaign_result.decision}"
        )
        ...
        if campaign_result.is_blocked:
            ...
            print(
                f"Layer 2B (Kids Input): {'[OK] SAFE' if is_safe else '[BLOCK] UNSAFE'}"
            )
    ...
    if is_on_topic:
        ...
        if not campaign_result.is_blocked:
            ...
            if not is_safe:
                print(
                    f"  -> [BLOCK] BLOCKED: {SafetyTemplates.get_template('UNSAFE_CONTENT')}"
                )
