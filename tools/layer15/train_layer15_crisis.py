...
class CrisisJSONL(Dataset):
    ...
    def __init__(self, path: str, tokenizer, max_len=256):
        self.rows = [
            json.loads(x) for x in open(path, "r", encoding="utf-8") if x.strip()
        ]
        self.tok = tokenizer
...
def main():
    ...
    ap.add_argument(
        "--outdir", default="models/layer15_crisis", help="Output directory"
    )
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        name, num_labels=3, problem_type="multi_label_classification"
    )
    ...
    for ep in range(args.epochs):
        ...
        print(
            f"[Epoch {ep + 1}/{args.epochs}] dev self_harm F2={f:.3f} P={prec:.3f} R={rec:.3f}"
        )
        ...
        if f > best_f:
            ...
            with open(os.path.join(args.outdir, "thresholds.json"), "w") as fjson:
                json.dump(
                    {"self_harm": float(thr_sh), "abuse": 0.5, "unsafe_env": 0.5}, fjson
                )
