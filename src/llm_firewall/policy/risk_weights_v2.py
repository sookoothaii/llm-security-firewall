...
FEATURE_FAMILIES = {
    'Transport': {'base64_secret', 'base85', 'rfc2047', 'armor_markers',
                  ...
                  # RC2 P4.2: Transport-Indicators Complete
                  'rfc2047_encoded', 'idna_punycode', 'fullwidth_b64', 'comment_split_b64', 'qp_multiline'},
    'Unicode': {'bidi_controls', 'zero_width_chars', 'fullwidth_forms', 'mixed_scripts'},
    'Decode': {'chain_decoded_1_stages', 'chain_decoded_2_stages', 'chain_decoded_3_stages',
               'yaml_anchors_neutralized', 'utf7_segments'},
    'Density': {'dense_alphabet', 'high_entropy'},
    'Grammar': {'yaml_anchors_neutralized', 'css_unescaped', 'js_unescaped', 'mime_headers_unfolded'},
        ...
        if hit in STRONG_SIGNALS or hit in STRONG_SIGNALS_EXTRA:
            ...
            if hit in {'high_entropy', 'dense_alphabet'}:
                ...
                if not passes_tri_key_gate(hit, hits):
                    ...
                    contributions[f'{hit}_suppressed'] = 'Tri-Key gate (no Transport/Unicode/STRONG)'
...
def decide_action(hits: list, context_meta: dict, text: str = '',
    ...
    if 'decoded_secretlike' in hits:
        ...
        if can_block:
            ...
            contrib['evidence_ladder_v2'] = 'Downgraded BLOCK→WARN (no risk proof in context)'
