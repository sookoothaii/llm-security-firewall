...
def collate_fn(
    batch: List[Tuple[str, torch.Tensor]], embedder: SentenceTransformer
...
def export_to_onnx(model: nn.Module, output_path: Path, input_dim: int = 384):
    ...
    logger.info(
        f"ONNX inference test: input {test_input.shape} -> output {outputs[0].shape}"
    )
...
def main():
    ...
    for epoch in range(50):
        ...
        logger.info(
            f"Epoch {epoch + 1:2d}: train_loss={train_loss:.4f}, val_loss={val_loss:.4f}"
        )
        for cat, m in metrics.items():
            logger.info(
                f"  {cat}: P={m['precision']:.3f}, R={m['recall']:.3f}, F1={m['f1']:.3f}"
            )
    ...
    logger.info(
        "[WARNING] Real deployment requires validated datasets with IRB approval!"
    )
