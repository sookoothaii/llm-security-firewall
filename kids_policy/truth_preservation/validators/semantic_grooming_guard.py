...
class SemanticGroomingGuard:
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            logger.info(
                f"🧠 SemanticGuard: Model loaded successfully. "
                f"{len(self.GROOMING_CONCEPTS)} threat concepts, "
                f"{len(self.BENIGN_CONCEPTS)} benign concepts. Layer B active."
            )
    ...
    def check_semantic_risk(
        ...
        try:
            ...
            # Finde das globale Maximum in der Matrix (threat similarity)
            ...
            if self._benign_embeddings is not None:
                benign_matrix = self._cosine_similarity(
                    embeddings, self._benign_embeddings
                )
                ...
                if max_benign_score > 0.95 and max_threat_score < 0.8:
                    ...
                    logger.debug(
                        f"[SemanticGroomingGuard] Extremely high benign similarity ({max_benign_score:.3f}, threat={max_threat_score:.3f}), "
