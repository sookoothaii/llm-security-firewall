...
try:
    ...
    if response_templates_path.exists():
        spec = importlib.util.spec_from_file_location(
            "response_templates", response_templates_path
        )
    ...
    if fallback_judge_path.exists():
        spec = importlib.util.spec_from_file_location(
            "fallback_judge", fallback_judge_path
        )
        if spec and spec.loader:
...
try:
    ...
    # JSONResponse not used, removed to fix lint
...
class LLMProxyServer:
    ...
    def __init__(self, config: Optional[ProxyConfig] = None):
        ...
        try:
            ...
            logger.info(
                f"Storage: Initialized ({'PostgreSQL' if storage_manager.is_postgresql else 'SQLite'})"
            )
        except Exception as e:
            logger.error(
                f"Storage: Failed to initialize: {e}. Falling back to in-memory only.",
                exc_info=True,
            )
            ...
            try:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(
                            f"Ollama connected: {self.config.ollama_url} (model: {self.config.ollama_model})"
                        )
                    else:
                        logger.warning(
                            f"Ollama connection test failed: {response.status_code}"
                        )
                        self.ollama_client = None
                except Exception as e:
                    logger.warning(
                        f"Ollama not available: {e}. Falling back to mock mode."
                    )
                    self.ollama_client = None
            except Exception as e:
        ...
        logger.info("🔒 RC10b detector: ✅ enabled")
        logger.info(
            f"👶 Kids Policy Truth Validator: {'✅ enabled' if self.truth_validator else '⚠️  disabled'}"
        )
        logger.info(
            f"🤖 Ollama integration: {'✅ enabled' if self.ollama_client else '⚠️  disabled (mock mode)'}"
        )
        if self.ollama_client:
            logger.info(
                f"   └─ Model: {self.config.ollama_model} @ {self.config.ollama_url}"
            )
        logger.info("=" * 70)
    ...
    def _get_or_create_memory(self, session_id: str) -> HierarchicalMemory:
        ...
        if storage_manager is not None:
            try:
                ...
                logger.warning(
                    f"Failed to save new session {session_id} to storage: {e}"
                )
    ...
    def _add_event_to_session(self, session_id: str, event: ToolEvent):
        ...
        if storage_manager is not None:
            try:
                ...
                logger.warning(
                    f"Failed to persist session {session_id} after event: {e}"
                )
    ...
    def _call_ollama(self, user_input: str) -> str:
        ...
        try:
            logger.debug(
                f"Calling Ollama API: POST /api/generate with model {self.config.ollama_model}"
            )
            response = self.ollama_client.post(
                "/api/generate", json=payload, timeout=30.0
            )
    ...
    def process_request(
        ...
        try:
            ...
            if not is_safe_input:
                logger.warning(
                    f"[Layer 0] UNSAFE_CONTENT detected for age_band {age_band}"
                )
        ...
        if not is_on_topic:
            ...
            logger.warning(
                f"[Layer 1] OFF_TOPIC detected: {best_topic} (score: {score:.3f})"
            )
        ...
        if (
            hasattr(inspection_result, "forced_phase")
            and inspection_result.forced_phase > 0
        ):
        ...
        if self.truth_validator and topic_id:
            ...
            try:
                ...
                is_safe_output = self.fallback_judge.evaluate_safety(
                    llm_output, age_band
                )
...
if HAS_FASTAPI:
    ...
    async def admin_stats():
        ...
        blocked_count = sum(
            1 for log in REQUEST_LOGS if "BLOCKED" in log.get("decision", "")
        )
        ...
        return {
            ...
            "active_sessions": len(
                SESSION_STORE
            ),  # Cache size (actual sessions may be in storage)
            "version": "1.0.0",
        }
    ...
    async def admin_memory(session_id: str):
        ...
        if memory is None and storage_manager is not None:
            try:
                ...
                logger.warning(
                    f"Failed to load session {session_id} from storage for admin: {e}"
                )
    ...
    def run_server(host: str = "0.0.0.0", port: int = 8081):  # nosec B104
