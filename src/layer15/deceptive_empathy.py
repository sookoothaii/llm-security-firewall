...
class DeceptiveEmpathyFilter:
    ...
    def rewrite(self, text: str, lang: str = "en") -> Tuple[str, bool]:
        ...
        stripped = re.sub(r"\b(ich\s+(sehe|h[öo]re|f[üu]hle)\s+dich)\b", "", stripped, flags=re.I)
        stripped = re.sub(r"\b(dear\s+friend|as\s+your\s+friend|als\s+dein\s+freund)\b", "", stripped, flags=re.I)
        stripped = re.sub(r"\b(as\s+a\s+therapist|als\s+therapeut)\b", "", stripped, flags=re.I)
        ...
        stripped = re.sub(r"\b(in\s+my\s+experience|aus\s+meiner\s+erfahrung)\b", "", stripped, flags=re.I)
