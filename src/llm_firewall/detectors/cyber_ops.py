...
from llm_firewall.lexicons.security_pretext import (
    load_security_pretext_lexicon,
)
...
CYBER_OFFENSIVE_PATTERNS = {
    ...
    "vulnerability research": ["vuln research", "vulnerability analysis", "vuln hunting"],
    
    # Attack techniques
    "credential harvesting": ["credential dumping", "password extraction", "hash extraction"],
    ...
    "we have authorization": ["we have permission", "authorized test", "legitimate test"],
...
def detect_red_team_persona(text: str, security_pretext_lexicon: Optional[Dict] = None) -> List[str]:
    """
    ...
    if security_pretext_lexicon is None:
        lexicon_path = Path(__file__).parent.parent / "lexicons" / "security_pretext.json"
...
def detect_authority_legitimacy_heuristic(text: str) -> float:
    ...
    authority_patterns = [
        "we are", "i am", "our company", "our team",
    ...
    # Legitimacy claims
    legitimacy_patterns = [
        "we have permission", "authorized test", "legitimate test",
        "we own this", "we control this", "assume we have access",
    ]
    
    legitimacy_count = sum(1 for pattern in legitimacy_patterns if pattern in text_lower)
