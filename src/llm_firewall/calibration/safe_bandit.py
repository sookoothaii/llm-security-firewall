...
def optimize_threshold_offline(scores: Iterable[float], labels: Iterable[int], fpr_max: float = 0.005) -> Metrics:
    ...
    for thr in qs:
        ...
        if m.fpr <= fpr_max:
            if best is None or m.asr < best[0] or (math.isclose(m.asr, best[0]) and thr < best[1]):
                best = (m.asr, thr)
                best_metrics = m
