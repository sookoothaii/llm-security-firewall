...
class ProcessingContext:
    ...
    is_likely_benign: bool = False  # FPR-Kontrolle
    context_type: str = "unknown"  # "technical", "educational", "casual", "suspicious"
...
class FirewallConfig:
    ...
    blocking_threshold: float = 0.19  # TUNED: 0.18 -> 0.19 (optimal balance: 0.18 zu aggressiv, 0.20 zu konservativ)
    ...
    # Toxicity parameters
    toxicity_threshold: float = 0.42  # TUNED: 0.38 -> 0.42 (konservativer für FPR-Reduktion)
    toxicity_low_threshold: float = 0.42  # For subtle toxicity
    toxicity_high_threshold: float = 0.45  # For explicit toxicity

    # Semantic Guard parameters
    semantic_threshold: float = 0.62  # TUNED: 0.60 -> 0.62 (konservativer für FPR-Reduktion)
    semantic_guard_low_threshold: float = 0.62  # For subtle threats
    ...
    min_threshold: float = 0.18  # TUNED: 0.15 -> 0.18 (konservativer)
    max_threshold: float = 0.25  # Maximum blocking threshold
    dynamic_tech_content_reduction: float = 0.03  # Reduktion für technische Inhalte
    ...
    meta_exploitation_confidence_threshold: float = 0.85  # Hoch für niedrige FPR
    
    # FPR-Optimierung
    fpr_optimization_mode: bool = True
    max_fpr_risk_increase: float = 0.10  # Maximaler Risikoanstieg pro Layer für low-risk prompts
...
class MetaExploitationDetectionLayer(SecurityLayer):
    ...
    def process(self, context: ProcessingContext) -> ProcessingContext:
        ...
        # FPR-OPTIMIZATION: Skip if prompt appears benign
        if (self.config.fpr_optimization_mode and 
            context.is_likely_benign and
            context.risk_score < 0.10):
            self._log_result(
                context, "SKIPPED", "Benign prompt detected, skipping meta-exploitation check"
            )
            return context
...
class FirewallEngineV3:
    ...
    def process_input(self, user_id: str, text: str, **kwargs) -> "FirewallDecision":
        ...
        # Pre-classify context for FPR optimization
        context.is_likely_benign = self._is_likely_benign(text)
        context.context_type = self._classify_context_type(text)
        ...
        for layer in self.input_layers:
            try:
                ...
                # FPR Control: Cap risk increases for benign prompts
                if (context.is_likely_benign and 
                    self.config.fpr_optimization_mode and
                    context.risk_score > original_risk):
                    risk_increase = context.risk_score - original_risk
                    if risk_increase > self.config.max_fpr_risk_increase:
                        context.risk_score = original_risk + self.config.max_fpr_risk_increase
                        self.logger.debug(
                            f"FPR control: Capped risk increase for benign prompt "
                            f"({risk_increase:.3f} -> {self.config.max_fpr_risk_increase})"
                        )
    ...
    def _calculate_dynamic_threshold(self, context: ProcessingContext) -> float:
        ...
        if self._is_technical_content(context.text):
            adjustments.append(-self.config.dynamic_tech_content_reduction)  # Use config value
