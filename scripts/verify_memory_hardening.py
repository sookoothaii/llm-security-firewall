    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            response = client.post(
                PROXY_URL,
                json={"message": message, "age_band": "9-12"},
...
def test_long_con():
    ...
    if multiplier >= 2.0:
        print("  [OK] Latent multiplier is >= 2.0 (Paranoid Mode active)")
    else:
...
def main():
    ...
    if test_passed:
        ...
        print(
            "   The system remains 'paranoid' even after critical events fall out of tactical buffer."
        )
        return 0
    else:
        print("\n[WARNING] Memory hardening test FAILED!")
        print(
            "   Review the output above. The latent risk multiplier may not be working correctly."
        )
        return 1
