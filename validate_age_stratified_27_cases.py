...
def main():
    ...
    for topic in TOPICS:
        ...
        for age in AGES:
            ...
            print(
                f"  Age band: {age} (mode: {cfg['validation']['nli']['bands'].get(age_band, 'unknown')})"
            )
            ...
            for culture in CULTURES:
                ...
                mnli_info = (
                    f"mnli={result['mnli']:.3f}" if result["mnli"] >= 0 else "mnli=N/A"
                )

                print(
                    f"    [{culture}] {status} ({mode}) {anchor_info} {mnli_info} - {result['reason']}"
                )
    ...
    for age in AGES:
        ...
        print(
            f"  {age}: {age_passed}/{age_total} ({100 * age_passed / age_total:.1f}%)"
        )
