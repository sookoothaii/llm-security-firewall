...
def run_sanity_checks(scenarios: List[CampaignScenario]) -> List[SanityCheckResult]:
    ...
    if "low_and_slow_single_target" in by_type:
        ...
        checks["single_target"] = all(
            calculate_unique_targets(s.events) == 1 for s in hc2_scenarios
        )
        ...
        if not low_diversity:
            errors.append(
                f"HC2 average tool diversity {avg_diversity:.1f} is not low (expected <= 4)"
            )
        ...
        if all_intervals:
            ...
            if not large_intervals:
                errors.append(
                    f"HC2 average event interval {avg_interval:.1f}s is not large (expected >= 3600s)"
                )
    ...
    if "bulk_recon_only" in by_type:
        ...
        if not many_targets:
            errors.append(
                f"HC3 average target count {avg_targets:.1f} is not high (expected >= 20)"
            )
    ...
    if "pretext_scope_abuse" in by_type:
        ...
        for i, scenario in enumerate(hc4_scenarios):
            ...
            has_external = any(
                ".com" in t or ".org" in t or ".net" in t for t in targets
            )
            if not (has_testlab and has_external):
                errors.append(
                    f"HC4 scenario {i} does not have mixed testlab+external targets"
                )
...
def calculate_difficulty_metrics(
    ...
    for difficulty, difficulty_scenarios in by_difficulty.items():
        malicious_scenarios = [
            s for s in difficulty_scenarios if s.label == CampaignLabel.MALICIOUS
        ]
        benign_scenarios = [
            s for s in difficulty_scenarios if s.label == CampaignLabel.BENIGN
        ]
        ...
        if malicious_results:
            ...
            avg_risk_malicious = statistics.mean(
                [r["risk_score"] for r in malicious_results]
            )
        else:
            asr = 0.0
            detected = 0
    ...
    for scenario in malicious_scenarios:
        ...
        for i in range(len(events)):
            ...
            if risk_score >= threshold and delay_events == -1:
                ...
                delay_time = (
                    events[i].timestamp - events[0].timestamp if events else 0.0
                )
        ...
        if not detected:
            ...
            delay_time = (
                events[-1].timestamp - events[0].timestamp if len(events) > 1 else 0.0
            )

        results.append(
            DetectionDelayResult(
...
def print_validation_report(results: Phase2ValidationResults):
    ...
    for difficulty in difficulty_order:
        ...
        print(
            f"  Malicious: {metrics.total_malicious} (detected: {metrics.detected_malicious}, blocked: {metrics.blocked_malicious})"
        )
        print(
            f"  Benign: {metrics.total_benign} (false positives: {metrics.false_positives})"
        )
    ...
    for difficulty in difficulty_order:
        ...
        if delay_events:
            print(
                f"  Delay (events): mean={statistics.mean(delay_events):.1f}, median={statistics.median(delay_events):.1f}"
            )
        if delay_times:
            print(
                f"  Delay (time): mean={statistics.mean(delay_times) / 3600:.2f}h, median={statistics.median(delay_times) / 3600:.2f}h"
            )
        detected_count = sum(1 for d in delays if d.detected)
        print(
            f"  Detected: {detected_count}/{len(delays)} ({detected_count / len(delays) * 100:.1f}%)"
        )
        print()
    ...
    if results.ablation_results:
        ...
        for ablation_name, ablation_metrics in results.ablation_results.items():
            ...
            for difficulty in difficulty_order:
                ...
                print(
                    f"  {difficulty.value}: ASR={metrics.asr:.3f}, FPR={metrics.fpr:.3f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.45, help="Risk score threshold"
    )
