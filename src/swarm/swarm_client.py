...
class SwarmClient:
    ...
    def __init__(self, bootstrap_nodes: Optional[List[str]] = None):
        ...
        if LIBP2P_AVAILABLE:
            ...
            logger.warning("Swarm Client: Using stub implementation (libp2p not available)")
    ...
    def on_threat_update(self, msg: Dict):
        ...
        try:
            ...
            if self.is_valid_signature(threat):
                ...
                logger.info(f"Swarm: Received threat update from peer {threat.peer_id[:8]}...")
            else:
                logger.warning(f"Swarm: Invalid signature for threat update from {threat.peer_id[:8]}...")
