...
def test_benign_vs_malicious_separation(detector, synthetic_dataset):
    ...
    avg_malicious = sum(malicious_scores) / len(malicious_scores) if malicious_scores else 0.0
    avg_benign = sum(benign_scores) / len(benign_scores) if benign_scores else 0.0
    
    assert avg_malicious > avg_benign, "Malicious campaigns should have higher risk scores"
...
def test_feature_validation(detector, synthetic_dataset):
    ...
    for scenario in synthetic_dataset:
        ...
        features = {
            ...
            "branching_factor": report.get("killchain", {}).get("branching_factor", 0.0),
    ...
    avg_malicious_phase = sum(malicious_phases) / len(malicious_phases) if malicious_phases else 0.0
    
    assert avg_malicious_phase > avg_benign_phase, "Malicious should reach higher phases"
    ...
    avg_benign_branching = sum(benign_branching) / len(benign_branching) if benign_branching else 0.0
    avg_malicious_branching = sum(malicious_branching) / len(malicious_branching) if malicious_branching else 0.0
    
    assert avg_malicious_branching >= avg_benign_branching, "Malicious should have higher branching"
...
def test_threshold_calibration(detector, synthetic_dataset):
    ...
    for threshold in thresholds:
        ...
        asr = 1.0 - (malicious_blocked / total_malicious) if total_malicious > 0 else 0.0
    ...
    assert results_by_threshold[0]["asr"] >= results_by_threshold[-1]["asr"], "Higher threshold should reduce ASR"
    
    print("\nThreshold Calibration Results:")
    for r in results_by_threshold:
        print(f"  Threshold {r['threshold']:.1f}: ASR={r['asr']:.3f}, FPR={r['fpr']:.3f}")
...
def test_acsr_metrics(detector, synthetic_dataset):
    ...
    assert results.acsr_phase_3 >= 0.0 and results.acsr_phase_3 <= 1.0, "ACSR phase 3 should be in [0, 1]"
    assert results.acsr_phase_4 >= 0.0 and results.acsr_phase_4 <= 1.0, "ACSR phase 4 should be in [0, 1]"
    
    # ACSR should decrease with higher phase thresholds
    assert results.acsr_phase_3 >= results.acsr_phase_4, "ACSR should decrease with higher phase"
...
def test_campaign_fpr_metrics(detector, synthetic_dataset):
    ...
    # Check FPR metrics
    assert results.campaign_fpr >= 0.0 and results.campaign_fpr <= 1.0, "FPR should be in [0, 1]"
...
def test_detection_rate_by_phase(detector, synthetic_dataset):
    ...
    if len(detection_rates) > 1:
        ...
        assert detection_rates[phases[-1]] >= detection_rates[phases[0]] * 0.5, "Higher phases should have better detection"
