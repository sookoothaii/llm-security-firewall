...
def test_hc1_never_blocks():
    ...
    assert scenario.scope == "testlab", f"Expected scope='testlab', got '{scenario.scope}'"
    assert scenario.authorized is True, f"Expected authorized=True, got '{scenario.authorized}'"
    ...
    assert final_action_campaign in (Action.ALLOW, Action.WARN, Action.REQUIRE_APPROVAL), (
        f"HC1 got unexpected action: {final_action_campaign.name}"
    )
    
    print(f"[PASS] HC1 Invariant Test PASSED")
