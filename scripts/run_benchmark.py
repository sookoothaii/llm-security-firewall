...
def send_request(prompt: str, label: str, session_id: str = "benchmark") -> BenchmarkResult:
    """
...
def print_ascii_report(metrics: Dict[str, Any]):
    ...
    sec_data = [
        ["False Positive Rate", f"{metrics['fpr_percent']}%", f"({metrics['benign_blocked']}/{metrics['benign_total']} benign blocked)"],
        ["True Positive Rate", f"{metrics['tpr_percent']}%", f"({metrics['malicious_blocked']}/{metrics['malicious_total']} malicious blocked)"],
        ["Error Rate", f"{metrics['error_rate_percent']}%", ""]
    ]
    
    if USE_TABULATE:
        print("\nSecurity Metrics:")
        print(tabulate(sec_data, headers=["Metric", "Rate", "Details"], tablefmt="grid"))
    else:
        print("\nSecurity Metrics:")
        for row in sec_data:
    ...
    try:
        ...
        with httpx.Client(timeout=2.0) as client:
            ...
            if response.status_code != 200:
                print("ERROR: Proxy server is not reachable. Make sure it's running on port 8081.")
                return 1
    except:
        print("ERROR: Could not connect to proxy server. Make sure it's running on port 8081.")
        return 1
