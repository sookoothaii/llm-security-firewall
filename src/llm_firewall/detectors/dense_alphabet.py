...
def dense_alphabet_flag(text: str, min_span_length: int = 30, density_threshold: float = 0.85):
    """
    ...
    DENSE_CHARS = set(string.ascii_letters + string.digits + "!#$%&()*+,-./:;<=>?@[]^_`{|}~\"'\\")
    ...
    return {
        ...
        'max_span_length': max(len(s) for s in high_density_spans) if high_density_spans else 0
    }
