...
def score_arm(text: str, arm: str, detectors: Dict[str, float]) -> float:
    ...
    if arm == "A0":
        ...
        for c in intents["clusters"]:
            ...
            per[cid] = sum(
                next((w for (p, w) in phrases if p == pat), 0.0) for (_, pat) in hits
            )
        ...
        intent = srt[0][1]
        margin = intent - (srt[1][1] if len(srt) > 1 else 0.0)
    elif arm in ("A2", "A3"):
        intent = float(res["intent"]["lex_score"])
        margin = float(res["intent"].get("margin", 0.0))
    else:
    ...
    if arm == "A3":
        try:
            ...
            if gate_by_calibration(art):
                ev = res["pattern"]["by_category"].get(
                    "obfuscation_encoding", 0.0
                ) + res["pattern"]["by_category"].get("unicode_evasion", 0.0)
                evden = 1.0 - math.exp(-ev / 3.0)
                feats = [
                    detectors.get("emb_sim", 0.0),
                    detectors.get("ppl_anom", 0.0),
                    detectors.get("llm_judge", 0.0),
                    intent,
                    margin,
                    patt,
                    evden,
