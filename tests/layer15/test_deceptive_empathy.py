...
def test_rewrite_transparency_en(cfg):
    ...
    out, changed = f.rewrite(
        "Oh dear friend, I see you. As your friend I will always be here.", lang="en"
    )
...
def test_rewrite_transparency_de(cfg):
    ...
    f = DeceptiveEmpathyFilter(cfg["deceptive_empathy_filter"])
    out, changed = f.rewrite("Ich sehe dich. Als dein Freund bin ich hier.", lang="de")
...
def test_no_change_when_clean(cfg):
    ...
    f = DeceptiveEmpathyFilter(cfg["deceptive_empathy_filter"])
    out, changed = f.rewrite(
        "I can provide information about coping strategies.", lang="en"
    )
