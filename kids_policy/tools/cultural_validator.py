...
def run_case(
    ...
    if HAS_REAL_VALIDATOR and validator is not None:
        ...
        try:
            ...
            slots_text = [
                s.get("text", str(s)) if isinstance(s, dict) else str(s) for s in slots
            ]
    ...
    fact_hits = sum(
        1
        for f in facts
        if any(w in answer_lower for w in f.get("text", "").lower().split()[:3])
    )
...
def pass_gates(age: str, m: Metrics) -> bool:
    g = GATES[age]
    return (
        m.entailment >= g.E_min
        and m.entailment_plus_neutral >= g.EN_min
        and m.recall >= g.recall_min
        and m.sps >= g.sps_min
        and m.veto_contradictions == 0
    )
...
def main(argv: List[str]) -> int:
    ...
    ap.add_argument(
        "--answers",
        default="kids_policy/answers/answers_cultural_v1_0_0.json",
        type=str,
    )
    ...
    for topic in TOPICS:
        ...
        for age in AGES:
            ...
            for culture in CULTURES:
                ...
                print(
                    f"    {culture}: E={m.entailment:.3f} SPS={m.sps:.3f} Recall={m.recall:.3f} {status}"
                )
            ...
            if len(combo_measures) == 3:
                ...
                print(
                    f"    CSI: E_gap={csi['CSI_E_gap']:.4f} SPS_gap={csi['CSI_SPS_gap']:.4f} Recall_gap={csi['CSI_Recall_gap']:.4f} {'[PASS]' if csi_pass else '[FAIL]'}"
                )
    ...
    for (topic, age), measures in per_topic_age.items():
        ...
        csi_pass = all(v <= 0.05 for v in csi.values())
        csi_report.append({"topic": topic, "age": age, **csi, "csi_pass": csi_pass})
