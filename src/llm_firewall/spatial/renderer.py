...
class SpatialCaptchaRenderer:
    ...
    def _render_2d_pil(self, challenge: Challenge) -> str:
        ...
        for obj in challenge.objects:
            ...
            if obj.object_type.value in ["cube", "pyramid"]:
                # Rectangle
                draw.rectangle([x-r, y-r, x+r, y+r], fill=obj.color, outline='black', width=2)
            else:
                # Circle
                draw.ellipse([x-r, y-r, x+r, y+r], fill=obj.color, outline='black', width=2)
        ...
        for i, opt in enumerate(challenge.options):
            draw.text((20, options_y + 30 + i*30), f"{chr(65+i)}. {opt}", fill='black')
    ...
    def _render_3d_matplotlib(self, challenge: Challenge) -> str:
        ...
        # Add options as text
        options_text = "Options:\n" + "\n".join([f"{chr(65+i)}. {opt}" for i, opt in enumerate(challenge.options)])
