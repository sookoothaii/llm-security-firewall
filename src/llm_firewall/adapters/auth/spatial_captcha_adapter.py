...
class SpatialCaptchaAdapter(AuthenticationPort):
    ...
    def _generate_new_challenge(self, user_id: str, context: Dict[str, Any]) -> AuthenticationResult:
        """Generate and present new challenge."""
        # Get user profile for adaptive difficulty
        profile = self._get_user_profile(user_id)
        difficulty = profile.recommend_difficulty() if profile else self.default_difficulty
    ...
    def _update_user_profile(self, user_id: str, challenge: Challenge, response: ChallengeResponse):
