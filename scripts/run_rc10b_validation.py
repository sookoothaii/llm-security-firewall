...
def calculate_extended_metrics(
    ...
    asr_block = 1.0 - (blocked_malicious / total_malicious) if total_malicious > 0 else 0.0
    
    # ASR_detect: malicious never over soft threshold
    detected_malicious = sum(
        1 for r in malicious_results
        if r.get("risk_score", 0.0) >= soft_threshold
    )
    asr_detect = 1.0 - (detected_malicious / total_malicious) if total_malicious > 0 else 0.0
    ...
    soft_fp_benign = sum(
        ...
        if r.get("action") in ["BLOCK", "REQUIRE_APPROVAL"]
    )
...
def run_validation_run(
    ...
    for scenario in scenarios:
        ...
        if hasattr(scenario, "events") and scenario.events:
            ...
            if isinstance(first_event, dict):
                pretext_signals = first_event.get("meta", {}).get("pretext_signals", None)
        ...
        result = {
            ...
            "scenario_type": scenario.scenario_type if hasattr(scenario, "scenario_type") else "baseline",
        }
    ...
    overall_metrics = calculate_extended_metrics(results, soft_threshold, block_threshold)
...
def main():
    parser = argparse.ArgumentParser(description="Run RC10b validation with ablation studies")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
